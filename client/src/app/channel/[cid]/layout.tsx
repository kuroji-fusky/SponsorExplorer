...
import { type ViewItemContext, ViewItemProvider, ChannelStoreProvider } from "@/context"
...
export async function generateMetadata(props: ChannelIdRouteParams): Promise<Metadata> {
...
export default async function ChannelLayout({ children, ...props }: Readonly<{ children: React.ReactNode } & ChannelIdRouteParams>) {
          ...
          {videos.length !== 0 ? (
            ...
            <div className="!mt-10 flex flex-col items-center space-y-9">
              ...
              <span className="max-w-screen-md text-center">
                This channel has no videos. If you believe this is a mistake, double check if you entered the channel ID, handle, or username correctly.
              </span>
            </div>
          )}
        </ViewItemProvider>
