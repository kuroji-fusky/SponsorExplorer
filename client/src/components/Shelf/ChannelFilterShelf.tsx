...
export function ChannelFilterShelf() {
  ...
  return (
    <section className="sticky top-14 z-40 py-4 bg-white  dark:bg-neutral-950 flex gap-x-1.5">
      <Tabs
        tabs={
          [{ label: "All" }, { label: "Videos" }, { label: "Shorts" }] as const
        }
        activeTab="Videos"
      />
      <div className="flex-1 flex items-center gap-x-2">
        <button
          ...
          className={cn(
            "transition-opacity",
            !isSegmentFetchComplete ? "opacity-65" : "",
          )}
        >
        ...
        {!isSegmentFetchComplete ? (
          <div
            id="spinner"
            className="animate-[spin_850ms_linear_infinite] size-4 rounded-full border-[3px] !border-l-transparent border-neutral-900 dark:border-neutral-50"
          ></div>
        ) : null}
      </div>
